 ...
 * 
   ...
   Diagnose button + automatic reset ........................ 1006  
...
// the number of the pushbutton pin for diagnose
const int diagnoseButton = D4;     
...
unsigned long previousMillis_3 = 0;        // periodical reset
const long interval_3 = 1*60*1000;         // interval at which reset counters (milliseconds)
int resetCounter=0;

int aliveCounter=10;                       // counter to reset ESP if no wifi connection is perceived
...
FrameCallback frames[] = {internalTemp,                             /* Internal Temp */
...
void setup() {
  ...
  pinMode(diagnoseButton, INPUT);                                   // initialize the pushbutton pin as an input
    ...
    // time budget.
          ...
          previousMillis_1 = currentMillis;                             // save the last time weather updated
            if(WiFi.status() != WL_CONNECTED) {
                if(aliveCounter<=0){ESP.reset();}
                aliveCounter--;
                Serial.print("Wifi not connected, reseting in few loops");
            } else {
            WiFiStatus();
            geolocationRequest();                                       // Geolocation by IP Request 
            requestCurrentWeather();
            aliveCounter=10;                                            // restart counter
            }
          } 
          ...
          if (currentMillis - previousMillis_3 >= interval_3) {         // periodical reset each 24h
          previousMillis_3 = currentMillis;                             // save the last time temperature updated
...
/***************************************************************************************
**                 Display CurrentWeather OR diagnose Wifi
***************************************************************************************/

void currentWeather(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {

if(digitalRead(diagnoseButton)==LOW){
...
void internalTemp(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
...
/***************************************************************************************
**              Display Screen FORECAST or Geolocation diagnose
***************************************************************************************/

void forecast(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
