...
FrameCallback frames[] = { drawScreen1, drawScreen2, drawScreen3, drawScreen4 }; // This array keeps function pointers to all frames
...
void drawScreen3(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->drawString(0, 0, Current_dt);
      ...
      display->drawString(0, 20, Current_temp);
          display->setFont(Dialog_plain_16);
          display->drawString(42, 40, "°C");
      display->setFont(ArialMT_Plain_10);
      display->drawString(70, 13, "FEELS");
...
void drawScreen4(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  dtostrf(temperatureC, 2, 1, buff);  //2 is mininum width, 1 is precision
      ...
      display->setFont(ArialMT_Plain_10);
      display->drawString(55, 13, "INT");
      display->setFont(Dialog_plain_40);           // Show int temp
      display->drawString(5, 17, int_temperatureC);
          display->setFont(Dialog_plain_16);
          display->drawString(95, 37, "°C");
