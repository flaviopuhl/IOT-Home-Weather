...
void drawScreen3(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->setFont(ArialMT_Plain_10);
  display->drawString(0, 0, Current_dt);

      display->setFont(ArialMT_Plain_24);           // Show ext temp
      display->drawString(0, 28, Current_temp);
          display->setFont(ArialMT_Plain_10);
          display->drawString(27, 40, "°C");

          display->setFont(ArialMT_Plain_16);       // Show feels like
          display->drawString(45, 26, Current_feels_like);

  //https://rickkas7.github.io/DisplayGenerator/index.html
  
  //display->setFont(ArialMT_Plain_10);
  //display->drawString(0, 13, "dt:"); display->drawString(30, 13, Current_dt);
  //display->drawString(0, 26, "temp :"); display->drawString(30, 26, Current_temp);
  //display->drawString(0, 39, "feels :"); display->drawString(30, 39, Current_feels_like);
